...
* Check the `ConnectionStrings` in `appsettings.json` files under the `Pg.CityHome.HttpApi.Host` and `Pg.CityHome.DbMigrator` projects and change it if you need.
...
Run `Pg.CityHome.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `Pg.CityHome.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `Pg.CityHome.HttpApi.Host`: ASP.NET Core API application that is used to expose the APIs to the clients.
